class JsInfo < ApplicationRecord
  ...
  def self.import_js(all_js, _url, test_id)
    js_id = []
    all_js.each do |js|
      _js = JsInfo.where(js_name: js[0], url_id: _url, status: true).first
      if _js
        ...
        if  _version != js[1]
          _js.status = false
          _js.save
          new_js = JsInfo.create(:first_seen => test_id, :last_seen => test_id, js_name: js[0], url_id: _url, status: true, version: js[1] )
          js_id << new_js.id
        else
