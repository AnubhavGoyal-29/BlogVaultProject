class Theme < ApplicationRecord
  ...
  def self.import_themes(themes, _url, test_id )
    ...
    themes.each do |theme|
      _theme = Theme.where(theme_name: theme, url_id: _url, status: true).first
      if _theme
        ...
        if  _version != '1.1'
          _theme.status = false
          _theme.save
          new_theme = Theme.create(:first_seen => test_id, :last_seen => test_id, theme_name: theme, url_id: _url, status: true, version: '1.1')
